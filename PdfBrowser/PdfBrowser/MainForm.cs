...
namespace PdfBrowser
{
    ...
    public partial class MainForm : Form
    {
        ...
        private const string FormatDatyIGodziny = "yyyy-MM-dd hh.mm.ss";
        ...
        {
            if (_indexesOfSelectedItems.Count > 0)
            {
                PdfFile file = _files[_indexesOfSelectedItems[0]];
        ...
        private void signButton_Click(object sender, EventArgs e)
        {
            if (_indexesOfSelectedItems.Count > 0)
            {
                ...
                else
                {
                                ...
                                if (ec2Xades.GetErrorCode == okCode)
                                {
                                    ...
                                    if (ec2Xades.GetErrorCode == okCode)
                                    {
                                        ...
                                        if (ec2Xades.GetErrorCode == okCode)
                                        {
                                            ...
                                            stringWriter.WriteLine(i + 1 + ".\t" + file.NazwaPliku + "\t" + statusDescription + "\tNumer referencyjny: " + referenceNumber);
                    ...
                    string reportPath = Path.Combine(reports, "wysylanie_" + DateTime.Now.ToString(FormatDatyIGodziny).Replace(':', '.') + ".txt");
                ...
                using (StringWriter stringWriter = new StringWriter(report))
                {
                    ...
                    for (int i = 0; i < _indexesOfSelectedItems.Count; i++)
                    {
                        ...
                        try
                        {
                            ...
                            stringWriter.WriteLine(i + 1 + ".\t" + file.NazwaPliku + "\t" + result + statusDescription);
                        }
                        catch (Exception exception)
                        {
                            stringWriter.WriteLine(i + 1 + ".\t" + file.NazwaPliku + "\tBłąd lokalny: " + exception.Message);
                    ...
                    string reports = Path.Combine(PdfFile.Katalog, "raporty");
                    string reportPath = Path.Combine(reports, "pobieranieUPO_" + DateTime.Now.ToString(FormatDatyIGodziny).Replace(':', '.') + ".txt");
