                                ...
                                case Enumeratory.Raport.NaleznosciSzczegolowoMiesiacWspolnoty:
                                    {
                                        ...
                                        int rok = data.Year;
                                        tytuł = String.Format("SZCZEGOLOWA ANALIZA NALEZNOSCI ZA M-C {0} - {1}", miesiąc, rok);
                                        DateTime początekMiesiąca = new DateTime(rok, miesiąc, 1);
                                        DateTime koniecMiesiąca = początekMiesiąca.AddMonths(1).AddSeconds(-1);
                                        IEnumerable<DostępDoBazy.Należność> należnościZaDanyMiesiąc = należności.Where(n => n.data_nal >= początekMiesiąca && n.data_nal <= koniecMiesiąca);
                                        ...
                                        switch (raport)
                                        {
                                            case Enumeratory.Raport.NaleznosciSzczegolowoMiesiacLokale:
                                                ...
                                                for (int i = 1; i <= lokale.Count; i++)
                                                {
                                                    DostępDoBazy.AktywnyLokal lokal = lokaleDoAnalizy[i - 1];
                                                    ...
                                                    tabela.Add(new string[] { i.ToString(), kodLok.ToString(), nrLok.ToString(), lokal.nazwisko, lokal.imie, String.Format("{0} {1}", lokal.adres, lokal.adres_2) });
                                                }
