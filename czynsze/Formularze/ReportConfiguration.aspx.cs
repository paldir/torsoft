...
namespace czynsze.Formularze
{
    public partial class ReportConfiguration : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            report = (Enumeratory.Raport)Enum.Parse(typeof(Enumeratory.Raport), key.Replace("report", String.Empty).Substring(key.LastIndexOf('$') + 1));
            ...
            placeOfConfigurationFields.Controls.Add(new Kontrolki.HtmlInputHidden(report + "report", "#"));
            ...
            switch (report)
            {
                case Enumeratory.Raport.LokaleWBudynkach:
                    using (DostępDoBazy.CzynszeKontekst db = new DostępDoBazy.CzynszeKontekst())
                    {
                        ...
                        Kontrolki.HtmlGenericControl firstBuilding = new Kontrolki.HtmlGenericControl("div", "control");
                        Kontrolki.HtmlGenericControl secondBuilding = new Kontrolki.HtmlGenericControl("div", "control");
                        ...
                        firstBuilding.Controls.Add(new Kontrolki.TextBox("field", "kod_1_start", firstBuildingNumber.ToString(), Kontrolki.TextBox.TextBoxMode.LiczbaCałkowita, 5, 1, true));
                        firstBuilding.Controls.Add(new Kontrolki.DropDownList("field", "kod_1_start_dropdown", buildings, firstBuildingNumber.ToString(), true, false));
                        secondBuilding.Controls.Add(new Kontrolki.TextBox("field", "kod_1_end", lastBuildingNumber.ToString(), Kontrolki.TextBox.TextBoxMode.LiczbaCałkowita, 5, 1, true));
                        secondBuilding.Controls.Add(new Kontrolki.DropDownList("field", "kod_1_end_dropdown", buildings, lastBuildingNumber.ToString(), true, false));
            ...
            controls.Add(new Kontrolki.RadioButtonList("list", "format", new List<string>() { "PDF", "CSV" }, new List<string>() { Enumeratory.FormatRaportu.Pdf.ToString(), Enumeratory.FormatRaportu.Csv.ToString() }, Enumeratory.FormatRaportu.Pdf.ToString(), true, false));
            labels.Add("Format: ");

            for (int i = 0; i < controls.Count; i++)
            {
                placeOfConfigurationFields.Controls.Add(new LiteralControl("<div class='fieldWithLabel'>"));
                placeOfConfigurationFields.Controls.Add(new Kontrolki.Label("label", controls[i].ID, labels[i], String.Empty));
